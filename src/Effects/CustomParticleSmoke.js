...
export class CustomParticleSmoke {
    constructor(camera, parentObject, texturePath = '/texture/smoke.png') {
        const uniforms = {
            diffuseTexture: {
        ...
        this._colourSpline.AddPoint(0.2, new THREE.Color(0xBBBBBB)); // رمادي متوسط فاتح
        this._colourSpline.AddPoint(0.5, new THREE.Color(0x888888)); // رمادي متوسط داكن
        this._colourSpline.AddPoint(0.8, new THREE.Color(0x555555)); // رمادي داكن
        this._colourSpline.AddPoint(1.0, new THREE.Color(0x1A1A1A));
        ...
        this.gdfsghk = 0.0; // تراكم الوقت لإضافة الجسيمات

        this._UpdateGeometry();
    }

    // لا نحتاج لـ _onKeyUp في هذا السياق (لأننا سنتحكم في اللهب عبر منطق المكوك)
    ...
    _AddParticles(timeElapsed) {
        ...
        const n = Math.floor(this.gdfsghk * 100.0); // 75 جسيم في الثانية
        this.gdfsghk -= n / 100.0;

        for (let i = 0; i < n; i++) {
            const life = (Math.random() * 0.75 + 0.25) * 15; // عمر الجسيمات
